...
def bikedata_to_od(data, col=['BIKE_ID', 'DATA_TIME', 'LONGITUDE', 'LATITUDE', 'LOCK_STATUS'], startend=None):
    ...
    move_data = oddata[(oddata[LOCK_STATUS] == 0) & (oddata[LOCK_STATUS+'_'] == 1)][[
        BIKE_ID, DATA_TIME, LONGITUDE, LATITUDE, DATA_TIME+'_', LONGITUDE+'_', LATITUDE+'_']]
    move_data.columns = [BIKE_ID, 'stime',
                         'slon', 'slat', 'etime', 'elon', 'elat']
    stop_data = oddata[(oddata[LOCK_STATUS] == 1) & (oddata[LOCK_STATUS+'_'] == 0)][[
        BIKE_ID, DATA_TIME, LONGITUDE, LATITUDE, DATA_TIME+'_', LONGITUDE+'_', LATITUDE+'_']]
