...
Typical process for spatiotemporal data analysis include preprocessing, spatiotemporal analysis, and data visualization. Currently, there exist lot of packages in each individual or multiply domains: 

- Data preprocessing : `MovingPandas` [@0x003aba2b] provide trajectory data structures and functions for data exploration and visualization on the top of `geopandas`.  `scikit-mobility` is a library that allow for managing and generate synthetic mobility data of various formats. `PTRAIL` is a library for mobility data preprocessing especially in feature generation and trajectory interpolation. `traffic` [@Olive2019] is a toolbox for processing and analyzing air traffic data. 
- Spatiotemporal analysis: `trackintel` [@axhausen2007definition] is a framework for spatio-temporal analysis of tracking data focusing on human mobility. `PySAL` [@rey_pysal_2010] is a family of packages that allows for advanced geospatial data science, which supports the development of high-level applications. 
- Data visualization: Apart from data structures,`MovingPandas` [@0x003aba2b] also support trajectory visualizations. `moveVis` [@2020moveVis] provides tools to visualize movement data and temporal changes of environmental data by creating video animations. `TraViA` [@Siebinga2021] provides tools to visualize and annotate movement data in an interactive approach. 

The above-mentioned libraries provide preprocessing and geometric analysis functions from sepcific aspects. However, there are still a lot of works to do while transforming raw spatio-temporal data into valuable insights. These tasks can hardly be fully covered by these libraries. For example, `geopandas` and `osmnx` [@boeing2017osmnx] are both mature library for processing complex geographic data expecially. A good compatibility with these tools is necessary for producing high-quality analysis. Thus, a library that integrates multiple data preprocessing, spatiotemporal analysis, and various visualization features through one programming language could effectively facilitate the research progress. 
